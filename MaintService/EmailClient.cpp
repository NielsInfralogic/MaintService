...
//	bool success = mailman.UnlockComponent(_T("INFRAL.CB1092019_4czEsHZS615x"));
	bool success = mailman.UnlockComponent(_T("NFRLGC.CB1112020_vZdFLucd1RAZ"));
...
//	bool success = mailman.UnlockComponent(_T("INFRAL.CB1092019_4czEsHZS615x"));
	...
	if (numMessages == -1) {
		m_lastError = mailman.lastErrorText();
		util.Logprintf(_T("POP3 ERROR: Error getting mail count - %s"), m_lastError);
	...
	if (bundle == NULL) {
		m_lastError = mailman.lastErrorText();
		util.Logprintf(_T("POP3 ERROR: Error copying mail to local storage - %s"), m_lastError);
		//mailman.Pop3EndSession();
		return -1;
	}
	CkString sUIDL;
...
//	bool success = mailman.UnlockComponent(_T("INFRAL.CB1092019_4czEsHZS615x"));
	...
	if (sAttachmentFile != _T("")) {
		...
		if (contentType == NULL) {
			m_lastError = email.lastErrorText();
			util.Logprintf(_T("SMTP WARNING: Unable to add attachment file %s - %s"), sAttachmentFile, m_lastError);
			return FALSE;
		}
	}
	...
	if (success != true) {
		m_lastError = email.lastErrorText();
		util.Logprintf(_T("SMTP ERROR: Unable to connect to smtp server %s - return code %s"), sSMTPServer, m_lastError);
		return FALSE;
	}
	else
	...
	if (success != true) {
		m_lastError = _T("Connection to SMTP server not closed cleanly");
		util.Logprintf(_T("SMTP ERROR: Connection to SMTP server not closed cleanly - %s"), m_lastError);
		return FALSE;
	}
...
CString CEmailClient::GetLastError()
{
	return m_lastError;
}