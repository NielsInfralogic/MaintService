...
TCHAR	g_szServiceName[MAX_PATH] = _T("MaintService");
TCHAR	g_szServiceDescription[MAX_PATH] = _T("PDFhub Maintenance Service");
...
BOOL InstallService()
{
	...
	keyClasses.Create(HKEY_LOCAL_MACHINE, _T("Software\\InfraLogic\\MaintService"));
	keyClasses.DeleteValue(_T("Parameters"));
	keyClasses.SetKeyValue(_T("Parameters"), _T("CC"), _T("DSN"));
	keyClasses.SetKeyValue(_T("Parameters"), _T("sa"), _T("DSNUser"));
	keyClasses.SetKeyValue(_T("Parameters"), _T("infra"), _T("DSNpw"));

	keyClasses.SetKeyValue(_T("Parameters"), (DWORD)0, _T("Logging"));
	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\ProcessService"), _T("LogFileFolder"));

	keyClasses.SetKeyValue(_T("Parameters"), (DWORD)15, _T("DatabaseLoginTimeout"));
	keyClasses.SetKeyValue(_T("Parameters"), (DWORD)15, _T("DatabaseQueryTimeout"));

	keyClasses.SetKeyValue(_T("Parameters"), (DWORD)5, _T("DatabaseQueryRetries"));
	keyClasses.SetKeyValue(_T("Parameters"), (DWORD)500, _T("DatabaseQueryBackoffTime"));

	keyClasses.SetKeyValue(_T("Parameters"), (DWORD)500, _T("DatabaseQueryBackoffTime"));

	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\MaintService\\Config.ini"), _T("ConfigFile"));
...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	strcpy(szEventStr, "MaintService");
	...
	if (g_prefs.LoadPreferencesFromRegistry() == FALSE)
		g_util.Logprintf("CANNOT READ Registry: Software\\InfraLogic\\MaintService\\Parameters..\r\n");
	else {
		g_util.Logprintf("INFO: Registry read: Software\\InfraLogic\\MaintService\\Parameters..\r\n");
		LogEvent(EVENTLOG_SUCCESS, IDS_REGISTRYREADOK, szEventStr, NULL, NULL);
	}

	// Startup 2: Export config file(s) from database to local ini-file


	CDatabaseManager dbmaint;
	CString sErrorMessage = _T("");
	CString sIniFile = _T("");
	CString sIniFile2 = _T("");
	...
	if (dbmaint.InitDB(g_prefs.m_DBserver, g_prefs.m_Database, g_prefs.m_DBuser, g_prefs.m_DBpassword, g_prefs.m_IntegratedSecurity, sErrorMessage) == FALSE) {
		g_util.Logprintf("Unable to connect to database - %s", sErrorMessage);
		LogEvent(EVENTLOG_SUCCESS, IDS_ERRORCONNDB, szEventStr, NULL, NULL);
	}
	else
		LogEvent(EVENTLOG_SUCCESS, IDS_CONNDB, szEventStr, NULL, NULL);

	
	if (dbmaint.LoadConfigIniFile(g_prefs.m_instancenumber, sIniFile, sIniFile2, g_prefs.m_setupEnableXMLpath, sErrorMessage))
		LogEvent(EVENTLOG_SUCCESS, IDS_CONNDB2, szEventStr, NULL, NULL);

	
	// Startup 3: Read the ini-preferences.
	if (sIniFile != "") {
		g_prefs.LoadIniFile(sIniFile);
