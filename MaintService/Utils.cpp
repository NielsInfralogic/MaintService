...
void CUtils::Logprintf(const TCHAR *msg, ...)
{
	...
	TruncateLogFile(g_prefs.m_logFilePath + _T("\\MaintService.log"), g_prefs.m_maxlogfilesize);

	HANDLE hFile = ::CreateFile(g_prefs.m_logFilePath + +_T("\\MaintService.log"), GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hFile == INVALID_HANDLE_VALUE)
		return;
...
BOOL CUtils::SaveFile(CString sFileName, CString sContent)
{
	...
	if (sContent.GetLength() > 4096)
		sContent = sContent.Left(4095);
	_tcscpy(szBuf, sContent);
	...
	BOOL ok = ::WriteFile(hFile, szBuf, _tcslen(szBuf), &nBytesWritten, NULL);
...
BOOL CUtils::CheckCRC(CString sFile, int nCRC)
{
	if (g_prefs.m_verifyCRC && nCRC > 0) {
		int nFileCRC = CRC32(sFile);
		if (nFileCRC > 0 && nCRC != nFileCRC) {
			Logprintf("WARNING: Checksum for file %s does not match database value (file=%d, DB=%d) - file not deleted", sFile, nFileCRC, nCRC);
			return FALSE;
		}
	}
